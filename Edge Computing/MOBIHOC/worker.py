...
def worker(info):
    # print("thread", info["who"].task_id, "starting....")
    target = info["who"]
    validation = []
    if not info["full"]:
        feasible = [0]
    ...
    # print("thread", target.task_id, "finish in", time.time() - start)

def check_worker(number_of_user):
    if np.sum(finish) == number_of_user:
        return True
    else:
        return False
...
def get_requests(item, selection):
    global request
    pool = [item]
    for req in request:
