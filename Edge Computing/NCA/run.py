...
def test(r):
    ...
    channels = []
    transmission_power_total = np.array([[ r.p_n_k[n][k] * r.get_ch_number(n)
    ...
    for n in range(r.number_of_user):
        for k in range(r.number_of_edge):
            if r.full_offload[n] > 0:
                ...
                allocated_f = r.X_n[n] / (r.D_n[n] - r.t_n_k[n][k] - r.ct_n_k[n][k])
                t_e.append(r.X_n[n] / allocated_f)
                rate = r.W * chs * math.log2(1 + r.p_n_k[n][k] * math.pow(r.H[n][k], 2) / r.N_0)
                e_n_k = r.p_n_k[n][k] * chs * (r.B[n] / rate) + r.k * r.Y_n[n] * math.pow(r.f_n_k[n][k], 2)

                ee.append(e_n_k)
                ee_c.append(r.k * r.Y_n[n] * math.pow(r.f_n_k[n][k], 2))
                ee_t.append(r.p_n_k[n][k] * chs * (r.B[n] / rate))
                # print("data rate", rate/8000)
                b[n] = round(rate * r.t_n_k[n][k], 0)
                # compare use max p
                pp = r.P_max[n] / chs
                ...
                f += r.X_n[n] / (r.D_n[n] - r.t_n_k[n][k] - r.ct_n_k[n][k])
                f_cpu[n] = r.X_n[n] / (r.D_n[n] - r.t_n_k[n][k] - r.ct_n_k[n][k])
            else:
                ee.append(ee_l[n])
                ee_t.append(0)
                ee_c.append(0)
    ...
    print("transmission      time", list(np.round(r.t_n_k.reshape(-1) + t_e + r.ct_n_k.reshape(-1), 4)))
    print("transmission      time", list(np.round(r.t_n_k, 4).reshape(-1)))
    print("edge  computation time", list(np.round(t_e, 4).reshape(-1)))
    print("local computation time", list(np.round(r.ct_n_k, 4).reshape(-1)))
    ...
    print("local allocated CPU", list(np.round(r.f_n_k / math.pow(10, 9), 4).reshape(-1)))
    print("max local       CPU", list(np.round(r.f_n / math.pow(10, 9), 4)))
    print(">>>>>>>>>>>>>>>>>>>>>>>>>>>data>>>>>>>>>>>>>>>>>>>>>>>>>>>>>")
    ...
    print("check >>> local computation data", list(np.round(r.f_n_k * r.ct_n_k / math.pow(10, 9), 5).reshape(-1)))
    print(">>>>>>>>>>>>>>>>>>>>>>>>>>>Energy>>>>>>>>>>>>>>>>>>>>>>>>>>>")
